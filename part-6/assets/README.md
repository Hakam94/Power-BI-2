...
9. `build_resolve_timeline.py` - Generates a **rough-cut `.fcpxml` timeline file** for Part 6: narration on the primary track, all 8 `generate_gemini_broll.py` clips on a track above it at their chapter timestamps, plus a marker per chapter. Works with **free** DaVinci Resolve (no Studio, no scripting API needed) — just `File > Import > Timeline...` the generated file in Resolve. Requires `ffprobe` (ships with ffmpeg) on PATH. Run: `python build_resolve_timeline.py --narration "path\to\trimmed_narration.mp4"`.
